...
from rmc.resources.basics import (
    filt_exac_cov_ht,
    filtered_exomes_ht_path,
    get_processed_context_ht_path,
    get_processed_gencode_ht_path,
    processed_exomes_ht_path,
)
from rmc.utils.utils import filter_alt_decoy, filter_to_missense, process_context_ht
...
def calculate_expected(context_ht: hl.Table, exac: bool) -> hl.Table:
    ...
    context_ht = context_ht.annotate(
        mu=(context_ht.mu_snp * (1 + (0.31898 * context_ht.div)))
    )
    ...
    exp_ht = exp_ht.transmute(
        possible_variants=exp_ht.variant_count
    )  # rename variant_counts (from count_variants function) to possible variants
    if exac:
        exp_ht = exp_ht.annotate(
            expected_variants=hl.case()
...
def calculate_observed(exome_ht: hl.Table, exac: bool) -> hl.Table:
    ...
    if exac:
        ...
        obs_ht = exome_ht.filter(
            (exome_ht.info.AC_Adj <= 123) & (exome_ht.info.VQSLOD >= -2.632)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exac", help="Use ExAC ht (not gnomAD ht)", action="store_true"
    )
    parser.add_argument(
        "--test",
